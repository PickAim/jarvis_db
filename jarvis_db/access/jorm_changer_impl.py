...
from jorm.market.service import FrequencyRequest, FrequencyResult, RequestInfo, UnitEconomyRequest, UnitEconomyResult
...
class JormChangerImpl(JORMChanger):
    ...
    def save_unit_economy_request(self, request: UnitEconomyRequest, result: UnitEconomyResult, request_info: RequestInfo, user_id: int) -> int:
        self.__economy_service.save_request(
            request_info, request, result, user_id, 0)
        return 0

    def save_frequency_request(self, request: FrequencyRequest, result: FrequencyResult, request_info: RequestInfo, user_id: int) -> int:
