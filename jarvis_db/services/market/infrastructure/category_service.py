...
class CategoryService:
    ...
    def create(self, category_entity: CategoryEntity, marketplace_id: int):
        category = Category(name=category_entity.name, marketplace_id=marketplace_id)
    ...
    def find_by_name(
        ...
        category = self.__category_repository.find_by_name(name, marketplace_id)
        if category is None:
            return None
    ...
    def find_all_in_marketplace(self, marketplace_id: int) -> dict[int, CategoryEntity]:
        categories = self.__category_repository.find_all_in_marketplace(marketplace_id)
        return {
    ...
    def filter_existing_names(self, names: Iterable[str], marketplace_id) -> list[str]:
        return self.__category_repository.filter_existing_names(
            list(names), marketplace_id
        )
